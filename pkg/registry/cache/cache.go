...
// New creates a new Cache. It returns an error if the cacheDir exists
// and contains unexpected files. It chooses a cache implementation based
// on the files it finds in the cache directory, with a preference for the
// latest iteration of the cache implementation.
func New(cacheDir string) (Cache, error) {
	entries, err := os.ReadDir(cacheDir)
	if err != nil && !errors.Is(err, os.ErrNotExist) {
		return nil, fmt.Errorf("detect cache format: read cache directory: %v", err)
	}
	boltCache := sets.NewString(boltDBFile, boltDigestFile)
	...
	if found.IsSuperset(boltCache) || len(entries) == 0 {
		return NewBolt(cacheDir), nil
	}
	if found.IsSuperset(jsonCache) {
