...
func createDBStore(dbPath string) *sqlite.SQLQuerier {
	...
	if err := injectBundlePaths(db); err != nil {
		logrus.Fatal(err)
	}
...
func injectBundlePaths(db *sql.DB) error {
	// Update the operatorbundle table by generating a bundlepath for each bundle.
	_, err := db.Exec(`UPDATE operatorbundle SET bundlepath = "example.com/" || name || "/" || name || ":" || version`)
	return err
}
...
func runMain(m *testing.M) (exitCode int, err error) {
	...
	defer func() {
		if rmErr := os.RemoveAll(tmpDir); err != nil {
			err = utilerrors.NewAggregate([]error{err, fmt.Errorf("couldn't remove test directory: %v", rmErr)})
		}
	}()
	...
	if err != nil {
		return 1, fmt.Errorf("failed to create json cache: %v", err)
	}
	...
	eg.Go(func() error {
		...
		if err := s2.Serve(lis); err != nil {
			return fmt.Errorf("failed to serve fbc json cache: %v", err)
		}
		return nil
	})
	exit := m.Run()
...
func TestGetBundleForChannel(t *testing.T) {
	strip := func(bundle *api.Bundle) *api.Bundle {
		return &api.Bundle{
			CsvName:    bundle.CsvName,
			CsvJson:    bundle.CsvJson,
			BundlePath: bundle.BundlePath,
		}
	}
	t.Run("Sqlite", testGetBundleForChannel(dbAddress, strip(etcdoperator_v0_9_2("alpha", false, false))))
	t.Run("FBCJsonCache", testGetBundleForChannel(jsonCacheAddress, strip(etcdoperator_v0_9_2("alpha", false, true))))
}
...
func TestListBundles(t *testing.T) {
	...
	t.Run("Sqlite", testListBundles(dbAddress,
		strip(etcdoperator_v0_9_2("alpha", true, false)),
		strip(etcdoperator_v0_9_2("stable", true, false))))
	t.Run("FBCJsonCache", testListBundles(jsonCacheAddress,
		strip(etcdoperator_v0_9_2("alpha", true, true)),
		strip(etcdoperator_v0_9_2("stable", true, true))))
}
...
func etcdoperator_v0_9_2(channel string, addSkipsReplaces, addExtraProperties bool) *api.Bundle {
	b := &api.Bundle{
		...
		BundlePath: "example.com/etcdoperator.v0.9.2/etcdoperator.v0.9.2:0.9.2",
